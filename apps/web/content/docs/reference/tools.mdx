...
description: Complete reference for all 221 Talome AI tools across 17 domains.
---

Talome's AI assistant has 221 registered tools organized into 17 domains. The active tool set is smaller because domains activate from settings and message routing. Each tool has a defined purpose, Zod-validated input parameters, and returns structured data. Tools are classified into three security tiers that determine when and how they can execute.

<Accordions>
<Accordion title="Sources">
Verified on 2026-06-13 by importing `apps/core/dist/ai/agent.js` to register domains, then reading `getAllDomains()`, `getAllToolMeta()`, and `getActiveRegisteredTools()` from `apps/core/dist/ai/tool-registry.js`. Source definitions live in `apps/core/src/ai/agent.ts` and `apps/core/src/ai/tool-registry.ts`.
</Accordion>
</Accordions>
...
- **MCP server**: imports `activeTools` from the agent, so it exposes the same configured-domain tool set rather than every registered tool.
...
| audiobookshelf | 9 | `audiobookshelf_url` | No |
...
| setup | 2 | None | Yes (first-run wizard) |

**Total: 221 registered tools across 17 domains.**
