---
title: MCP Server
description: Use Talome's active tools from Claude Code, Cursor, or Claude Desktop.
---

Talome includes a built-in [Model Context Protocol](https://modelcontextprotocol.io) (MCP) server that exposes the active Talome tool set to external AI clients. Manage your home server from Claude Code, Cursor, Claude Desktop, or any MCP-compatible client without opening the Talome dashboard.
...
The MCP server imports `activeTools` from the agent, which is built from `getActiveRegisteredTools()`. Tools for an integration only load when that integration is configured. For example, the 27 `arr_*` tools require one of `sonarr_url`, `radarr_url`, `readarr_url`, or `prowlarr_url`. The core, mDNS, and setup domains are always active because their `settingsKeys` arrays are empty.

This keeps the per-session tool count lower than the 221-tool full registry. On the live setup used for this documentation pass, 120 tools were active.

<Accordions>
<Accordion title="Sources">
`apps/core/src/routes/mcp.ts` imports `activeTools` from `apps/core/src/ai/agent.ts`. `activeTools` is created with `getActiveRegisteredTools()` in `apps/core/src/ai/agent.ts`. Domain activation is implemented in `apps/core/src/ai/tool-registry.ts`.
</Accordion>
</Accordions>
...
The full registry currently contains 221 tools organized into 17 domains. MCP exposes the active subset for the current Talome configuration. The `core` domain is broken out by capability for readability — those are all part of the always-loaded core registration in `apps/core/src/ai/agent.ts`.

| Domain | Count | Description | Example tools |
|--------|-------|-------------|---------------|
| **core** | 114 | Containers, system stats, app lifecycle, settings, memories, files, automations, notifications, widgets, self-improvement, app creation | `list_containers`, `install_app`, `get_system_stats`, `remember`, `browse_files`, `create_automation`, `send_notification`, `plan_change`, `apply_change`, `design_app_blueprint`, `create_widget_manifest` |
...
<Cards>
  <Card title="Tools Reference" href="/docs/reference/tools">
    Complete list of all 221 registered tools with parameters.
  </Card>
