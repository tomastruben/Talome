...
Talome is an AI-first, open-source home server management platform. It combines a Docker container orchestrator, a multi-source app store (Talome + CasaOS + Umbrel + custom), and an agentic AI assistant with 221 registered tools — all in a single self-hosted package. You install, configure, and manage apps through natural language conversations.
...
| **AI assistant** | 221 registered tools, autonomous care, self-improvement | None | None | None |
...
| OpenAI | GPT-5.3 / GPT-5.4 / GPT-5.4 mini | ~$0.01-0.10/conversation |
...
- **TMDb image CDN.** Poster art (`image.tmdb.org`) is fetched lazily for watchlist items returned by Plex's metadata API.
...
- **GitHub** (`github.com`, `raw.githubusercontent.com`), when a third-party app store is enabled. Talome clones public CasaOS / Umbrel / BigBear repositories on each store sync so it can list those apps. Disable a store and the sync stops.
...
**Tool boundaries** — The AI can only perform actions defined by its registered tools. All tool inputs are validated with Zod schemas. `eval()` is never used. The agent does have a `run_shell` tool (so it can do real diagnostics on your behalf), but it's gated by Talome's three-mode security gateway: in *cautious* mode (the default for unattended operations) it's restricted to a vetted allowlist; in *locked* mode it's disabled entirely; in *permissive* mode every shell call is still audit-logged and surfaced to the user.
...
MCP (Model Context Protocol) is an open standard for connecting AI models to tools. Talome's MCP server exposes Talome's active tool set to external AI clients — Claude Code, Cursor, Claude Desktop, or any MCP-compatible tool.
...
Cloud models (Claude Sonnet 4.6, GPT-5.3) respond in 1-3 seconds regardless of your hardware and handle complex multi-tool tasks better. For most users, the best setup is a cloud provider for daily use and Ollama as a free fallback.
