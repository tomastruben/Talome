...
1. `getAllRegisteredTools()` returns a flat `Record<string, Tool>` of all 219 tools
...
**Tradeoffs:**
- The MCP server loads 219 tools at startup, which adds slight startup latency (~50-100ms). This is not noticeable in practice since the server starts once per Claude Code session.
