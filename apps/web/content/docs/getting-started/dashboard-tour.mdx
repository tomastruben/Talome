...
Screenshots on this page were captured from a running Talome dashboard at `http://localhost:3000` on 2026-06-13.
...
| Chat | **Assistant** | AI chat, suggested tasks, and conversation history |
| Film | **Media** | Unified media library and downloads |
| Book | **Audiobooks** | Audiobook library views |
| Folder | **Files** | File browser for server and mounted drives |
| Box | **Services** | Docker app and container management |
| Grid | **App Store** | Multi-source app store |
| Clock | **Automations** | Scheduled tasks and event triggers |
| Sparkles | **Intelligence** | Activity, diagnostics, and assistant-derived insights |
| Bug | **Bug Hunt** | Issue-hunting workflow entry point |
| Terminal | **Terminal** | Persistent tmux shell and Codex |
...
Talome currently ships with **25 built-in widget components** plus shared widget infrastructure. This count comes from `apps/dashboard/src/components/widgets/*-widget.tsx` and excludes helper files such as `widget-grid.tsx`.
...
- **Audiobooks** — Audiobookshelf library + listening progress
- **Overseerr Requests** — pending and recent media requests
- **Optimization** — transcoding queue + savings summary
...
- **Backup Status** — last/next backup, recent failures
- **Ollama Status** — local Ollama models + running processes
- **Launcher** — pinned app shortcuts
...
- **Declarative** — generic AI-generated widget (data + chart spec from a tool result)
...
![Talome app store](/docs/screenshots/talome-app-store.jpg)
...
The Services page gives you direct visibility into every Docker app and container running on your server — not just apps installed through Talome.
...
Click **Launch Codex** to start an interactive Codex session inside the terminal. This gives you the full agentic AI coding experience — Codex reads the Talome codebase, uses MCP tools, and can make changes to the source code interactively.

The terminal and Codex share the same tmux session (`talome-Codex`), so you can switch between shell commands and AI-assisted development.
...
![Talome settings](/docs/screenshots/talome-settings.jpg)
...
<Cards>
  ...
  <Card title="AI Assistant Guide" href="/docs/guides/ai-assistant">
    Learn the full scope of what the AI can do — 221 registered tools across 17 domains.
  </Card>
</Cards>

<Accordions>
<Accordion title="Sources">
`apps/dashboard/src/app/dashboard/` defines the dashboard routes. `apps/dashboard/src/components/app-sidebar.tsx` defines the sidebar labels. `apps/dashboard/src/components/widgets/` contains the widget components. `apps/dashboard/src/components/assistant/` and `apps/dashboard/src/app/dashboard/assistant/page.tsx` implement the assistant UI. `apps/dashboard/src/components/settings/` implements the settings sections.
</Accordion>
</Accordions>
