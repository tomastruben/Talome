...
If you prefer OpenAI, set your `OPENAI_API_KEY` and select OpenAI as the default provider. GPT-4o is the recommended model. All tools work identically — the AI layer is provider-agnostic.
...
Not all 219 tools are sent to the AI on every message. Talome uses **per-message tool routing**:
