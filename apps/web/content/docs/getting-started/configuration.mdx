...
Anthropic's Claude is Talome's default provider. All 219 tools are tested primarily against Claude models.
...
GPT-5.3 is the recommended model for a good balance of capability and cost. GPT-5.4 is available for heavier reasoning tasks. `gpt-4o-mini` is also supported and still works (no sunset date) — it's the codebase default if you pick OpenAI without explicitly choosing a model. For newer releases, go to **Settings > AI Provider** and pick from the live model list (Talome fetches this from OpenAI's `/v1/models` endpoint, so it always reflects what your account has access to).
...
<Cards>
  <Card title="AI Assistant Guide" href="/docs/guides/ai-assistant">
    Deep dive into the 219 tools, memory system, conversation patterns, and how dynamic tool loading works.
  </Card>
