...
Anthropic's Claude is Talome's default provider. The built-in Anthropic catalog currently lists Haiku and Sonnet in `apps/core/src/routes/ai-models.ts`.
...
Talome fetches OpenAI models from OpenAI's `/v1/models` endpoint and filters the list to chat-capable model families. Choose from the live list in **Settings > AI Provider**; availability depends on your OpenAI account.
...
3. In Talome Settings, set **Ollama URL** to `http://localhost:11434`
4. Select Ollama as the default provider

Talome ships as a native service (not a container), so `localhost:11434` reaches the Ollama you just installed. If you went out of your way to run Talome inside Docker (not supported — disables self-evolution), use `http://host.docker.internal:11434` instead.
...
<Callout type="info">
Local models handle single-tool tasks well (install an app, check status, read logs). Talome uses a shorter local prompt and native Ollama streaming for Ollama-backed chat, but multi-step operations can still be less reliable than larger cloud models. Start with Anthropic if you want the most tested path, then try Ollama once you are familiar with the system.
</Callout>

<Accordions>
<Accordion title="Sources">
`apps/core/src/routes/ai-models.ts` defines the Anthropic catalog, fetches OpenAI models from `/v1/models`, and reads Ollama models from the configured `ollama_url`. `apps/core/src/routes/chat.ts` contains the native Ollama `/api/chat` streaming path. `apps/core/src/ai/agent.ts` contains the provider-backed chat path and local-model tool selection.
</Accordion>
</Accordions>
...
All Talome state lives in a single SQLite database. On a native install (the supported path) it lives at `~/.talome/data/talome.db`. The exact path is configurable via the `DATABASE_PATH` env var in `~/.talome/server/apps/core/.env`.
...
```bash
# Stop the service first so SQLite isn't mid-write
sudo systemctl stop talome     # Linux
launchctl bootout gui/$(id -u) ~/Library/LaunchAgents/dev.talome.plist   # macOS

cp ~/.talome/data/talome.db ~/talome-backup.db

# Restart
sudo systemctl start talome    # Linux
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/dev.talome.plist  # macOS
```

Talome also takes automatic snapshots into `~/.talome/backups/` once a day via the built-in self-backup service (see [Installation → Updating](/docs/getting-started/installation#updating) for the retention knobs).
...
<Cards>
  <Card title="AI Assistant Guide" href="/docs/guides/ai-assistant">
    Deep dive into the 221 registered tools, memory system, conversation patterns, and how dynamic tool loading works.
  </Card>
