...
description: Your server, one message away. AI-powered home server management with 221 registered tools, a multi-source app store, and a self-improving codebase.
---
...
Talome's assistant is not a chatbot bolted onto a dashboard. It is the primary interface. The current registry contains 221 purpose-built tools across 17 domains, giving it structured access to Docker containers, app configurations, media libraries, network routing, storage health, and its own source code. It does not generate suggestions for you to follow. It executes.

When you say "set up a media stack," the AI can use app-store tools, app lifecycle tools, and dedicated media integration tools such as `wire_apps`, `arr_add_root_folder`, and `arr_sync_indexers_from_prowlarr`, then report back with URLs. That is not autocomplete. That is an agent with real tools and real authority over your infrastructure.

Tools are organized into domains that activate dynamically. Configure Sonarr, Radarr, Readarr, or Prowlarr, and the 27 arr tools become active. Configure Home Assistant, and 5 smart home tools become active. The AI only sees tools it can actually use, which keeps tool selection more focused.
...
Verified on 2026-06-13 from source. `apps/core/src/ai/agent.ts` registers 17 domains via `registerDomain()`. `apps/core/src/ai/tool-registry.ts` exposes `getAllToolMeta()` and `getActiveRegisteredTools()`; importing the built registry returned 221 registered tools and 120 active tools on the live setup used for screenshots. `apps/core/src/ai/tool-gateway.ts` enforces the three security modes. `apps/core/src/stores/lifecycle.ts` and `apps/core/src/app-registry/auto-configure.ts` implement install and post-install behavior.
</Accordion>
</Accordions>
...
These screenshots were captured from a running Talome dashboard at `http://localhost:3000` on 2026-06-13.

![Talome dashboard home](/docs/screenshots/talome-dashboard-home.jpg)

![Talome assistant](/docs/screenshots/talome-assistant.jpg)
...
<Cards>
  ...
  <Card title="221 registered AI tools" href="/docs/guides/ai-assistant">
...
- **`apps/core`** — Hono backend serving the API, AI agent, 221 registered tools, Docker orchestration, SQLite database, automation engine, and MCP server
...
Most Talome application state lives in SQLite — users, conversations, memories, automations, widgets, audit logs, settings, and app records. App data itself remains in each app's Docker volumes. The backend talks to Docker via the Unix socket. The AI communicates with Anthropic Claude, OpenAI, or local Ollama models. The MCP server exposes the active Talome tool set to external AI environments such as Claude Code, Cursor, and Claude Desktop.

<Accordions>
<Accordion title="Source">
`apps/core/src/db/schema.ts` defines the SQLite tables. `apps/core/src/routes/mcp.ts` imports `activeTools` from `apps/core/src/ai/agent.ts`, and `activeTools` is built from `getActiveRegisteredTools()` in `apps/core/src/ai/tool-registry.ts`.
</Accordion>
</Accordions>
...
<Cards>
  ...
  <Card title="MCP Server" href="/docs/reference/mcp-server">
    Use Talome's active tool set from Claude Code, Cursor, or Claude Desktop.
  </Card>
</Cards>
