...
description: Your server, one message away. AI-powered home server management with 220 tools, a multi-source app store, and a self-improving codebase.
---
...
Talome's assistant is not a chatbot bolted onto a dashboard. It is the primary interface. 220 purpose-built tools across 17 domains give it deep, structured access to your system — Docker containers, app configurations, media libraries, network routing, storage health, and its own source code. It does not generate suggestions for you to follow. It executes.
...
<Cards>
  ...
  <Card title="220 AI tools" href="/docs/guides/ai-assistant">
  ...
  <Card title="12 deep integrations" href="/docs/integrations">
    Sonarr, Radarr, Prowlarr, Jellyfin, Plex, qBittorrent, Overseerr, Home Assistant, Pi-hole, Audiobookshelf, Vaultwarden, and Ollama — each with dedicated tools, not just API wrappers.
  </Card>
</Cards>
...
- **`apps/core`** — Hono backend serving the API, AI agent, 220 tools, Docker orchestration, SQLite database, automation engine, and MCP server
...
All state lives in a single SQLite database — users, conversations, memories, automations, widgets, audit logs, app records. The backend talks to Docker via the Unix socket. The AI communicates with Anthropic Claude (recommended), OpenAI, or local Ollama models through the Vercel AI SDK. The MCP server exposes every tool to external AI environments — Claude Code, Cursor, and Claude Desktop can manage your server with the same 220 tools the dashboard uses.
...
<Cards>
  ...
  <Card title="MCP Server" href="/docs/reference/mcp-server">
    Use all 220 Talome tools from Claude Code, Cursor, or Claude Desktop.
  </Card>
</Cards>
